...
int main (int argc, char ** argv) {
	...
	if (nfiles) {				    /* find any input? */
		...
		s2[0] = m2 + 'a';
		s3[0] = m3 + 'a';
		printf ("static short maxcell = %ld; /* %s%s%s */\n", max, s1, s2, s3);

		printf ("static short sing[26] = {");
		for (c1=0; c1 < 26; c1++) {
			printf ("%d, ", sing[c1]);
		}
		printf ("};\n");				/* oops, ends in comma closebrace */

		printf ("static short duos[26][26] = {");
		for (c1=0; c1 < 26; c1++) {
			s1[0] = c1+'A';
			printf ("\n/* %s */ ", s1);
			for (c2=0; c2 < 26; c2++) {
		...
		printf ("static short tris[26][26][26] = {");
		for (c1=0; c1 < 26; c1++) {
			for (c2=0; c2 < 26; c2++) {
				s1[0] = c1+'A';
				s2[0] = c2+'A';
