 ...
 * rev 6ab75454 20260926 C port of cksh bash function rev 68e9ff40
 * (c) 2017-2026 George Georgalis <george@iuxta.com> Unlimited use with attribution.
 *
 * Output: inode links shake256 size mdate filename (hex, awk-compatible widths)
 *
 * Dependency posture: none beyond libc. SHAKE256 (FIPS 202) is implemented here
 * rather than linked from libcrypto; the bash revision depended on openssl
 * shake256 -xoflen, which OpenSSL < 3 and LibreSSL lack. An XOF output of
 * length L is a prefix of any longer output, so every -x 1-64 is exact on any
 * host. Static on Linux and NetBSD; Darwin links libSystem only.
 *
 * Divergence from 68e9ff40 (shared with the bash revision): non-regular nodes
 * get a zero-filled hash, fifos are never opened, per-file failures warn and
 * continue, and the exit status is a bitmask (see --help).
 */
...
warnf(const char *what, const char *name, int err)
{
	...
	(void)fprintf(stderr, "%s: %s%s%s%s%s%s\n", prog, what,
	    name ? " '" : "", name ? name : "", name ? "'" : "",
	    err ? ": " : "", err ? strerror(err) : "");
}

static void
die(int st, const char *what, const char *name)
{
	warnf(what, name, 0);
	exit(st);
}
...
hash_col(const char *name, const struct stat *lst, char *hex, size_t xlen)
{
	...
	if (S_ISLNK(lst->st_mode) && stat(name, &st) != 0) {
		warnf("dangling symlink, not hashed", name, 0);
		status |= ST_DANGLE;
		return 0;
	}
	...
	if (!S_ISREG(st.st_mode)) {
		warnf(S_ISFIFO(st.st_mode) ? "fifo, not hashed" :
		    S_ISSOCK(st.st_mode) ? "socket, not hashed" :
		    "special file, not hashed", name, 0);
		status |= ST_SPECIAL;
		return 0;
	}
	...
	if (fd < 0) {
		err = errno;
		warnf("cannot open", name, err);
		status |= ST_READ;
		return -1;
	}
	if (fstat(fd, &fst) != 0 || !S_ISREG(fst.st_mode)) {
		(void)close(fd);
		warnf("changed during scan, not hashed", name, 0);
		status |= ST_SPECIAL;
		return 0;
	}
	fl = fcntl(fd, F_GETFL);
	if (fl == -1 || fcntl(fd, F_SETFL, fl & ~O_NONBLOCK) == -1) {
		(void)close(fd);
		warnf("cannot set blocking read", name, 0);
		status |= ST_READ;
		return -1;
	}
...
xwrite(const char *s, size_t len)
{
	if (fwrite(s, 1, len, stdout) != len)
		die(ST_ENV, "write error on stdout", NULL);
}
...
keep(char *line)
{
	if (nrec == caprec) {
		...
		if (ncap < caprec || ncap > SIZE_MAX / sizeof(*rec))
			die(ST_ENV, "too many records", NULL);
		p = realloc(rec, ncap * sizeof(*rec));
		if (p == NULL)
			die(ST_ENV, "out of memory", NULL);
		rec = p;
		caprec = ncap;
	}
	rec[nrec++] = line;
...
/* format one output line: fields per -n, '.' for omitted, then decorated name */
static void
do_file(const char *name)
{
	char pre[256], hex[2 * XMAX + 1], f1[24], f2[24], f4[24], f5[24];
	const char *sfx = "";
	struct stat lst;
	size_t plen, nlen, tot;
	char *line;
	int n;

	if (lstat(name, &lst) != 0) {
		int err = errno;
		warnf("cannot stat", name, err);
		status |= ST_STAT;
		return;
	}
	if (opt_x > 0 && hash_col(name, &lst, hex, (size_t)opt_x) != 0)
		return;
	strcpy(f1, "."); strcpy(f2, "."); strcpy(f4, "."); strcpy(f5, ".");
	if (opt_n < 1)
		(void)snprintf(f1, sizeof(f1), "%8jx", (uintmax_t)lst.st_ino);
	if (opt_n < 2)
		(void)snprintf(f2, sizeof(f2), "%2jx", (uintmax_t)lst.st_nlink);
	if (opt_n < 4)
		(void)snprintf(f4, sizeof(f4), "%8jx", (uintmax_t)lst.st_size);
	if (opt_n < 5)
		(void)snprintf(f5, sizeof(f5), "%08jx", (uintmax_t)lst.st_mtime);
	n = snprintf(pre, sizeof(pre), "%s %s %s %s %s ", f1, f2,
	    opt_x > 0 ? hex : ".", f4, f5);
	if (n < 0 || (size_t)n >= sizeof(pre))
		die(ST_ENV, "internal format overflow", name);
	plen = (size_t)n;
	...
	nlen = strlen(name);
	if (nlen > SIZE_MAX - plen - 3)
		die(ST_ENV, "name too long", NULL);
	tot = plen + nlen + strlen(sfx);
	if (opt_k == 0 && opt_r == 0) {
		xwrite(pre, plen);
		xwrite(name, nlen);
		xwrite(sfx, strlen(sfx));
		xwrite("\n", 1);
		return;
	}
	line = malloc(tot + 1);
	if (line == NULL)
		die(ST_ENV, "out of memory", NULL);
	memcpy(line, pre, plen);
	memcpy(line + plen, name, nlen);
	strcpy(line + plen + nlen, sfx);
	keep(line);
}

/* --- sort, as LC_ALL=C sort [-r] -k N ------------------------------------ */

/* offset of field k (1-based), POSIX default fields: leading blanks included */
static const char *
field(const char *s, int k)
{
	int f;
...
cmp_rec(const void *a, const void *b)
{
	...
	c = strcmp(field(x, opt_k), field(y, opt_k));
	if (c == 0)
		c = strcmp(x, y);	/* last-resort whole-line comparison */
	return opt_r ? (c < 0) - (c > 0) : c;
}
...
flush_rec(void)
{
	...
	for (i = 0; i < nrec; i++) {
		const char *s = rec[opt_k || !opt_r ? i : nrec - 1 - i];
		xwrite(s, strlen(s));
		xwrite("\n", 1);
	}
	for (i = 0; i < nrec; i++)
...
{
	warnf(what, arg, 0);
	(void)fprintf(stderr, "%s: -h for usage, --help for the manual\n", prog);
	exit(ST_USAGE);
}
...
parse(int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++) {
		...
		for (p = w + 1; *p; p++) {
			...
			if (c == 'n') {
				v = arg ? num(arg, 0, 5) : 0;
				if (v < 0) bad("-n requires 0-5, got", arg);
				opt_n = v;
			} else if (c == 'x') {
				v = arg ? num(arg, 0, XMAX) : 0;
				if (v < 0) bad("-x requires 0-64, got", arg);
				opt_x = v;
			} else {
				v = arg ? num(arg, 1, 6) : 6;
				if (v < 0) bad("-k requires 1-6, got", arg);
				opt_k = v;
			}
	...
	if (i < argc) {
		...
		while ((len = getline(&ln, &cap, stdin)) != -1) {
			...
			if (len > 0 && strlen(ln) == (size_t)len)
				...
				warnf("NUL in input line, skipped", NULL, 0);
				status |= ST_STAT;
			}
		}
		if (ferror(stdin))
			die(ST_ENV, "read error on stdin", NULL);
		free(ln);
	}
	flush_rec();
