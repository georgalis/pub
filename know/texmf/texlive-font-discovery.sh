...
# revs: 6813b4a0 20260407 --- filter logic fix, figure reference appendix, legend
...
#   families showcased only in reference appendix, clearpage every 30 entries
set -euo pipefail
...
# --- collect enc:fam pairs, filter encoding + bitmap only ---
# partials (Inf/Sup/Dnom/Numr/Orn) are retained for the reference appendix
declare -a all_pairs=()
for r in "${roots[@]}" ; do
...
awk -F'\t' '!seen[$1]++ { printf "\\showfont{%s}{%s}\n", $4, $5 }' \
  < <(sort -t'	' -k1,1 -k2,2n -k3,3n \
    < <(awk -F: '{
          enc = $1; fam = $2
          # exclude partials from showcase (rendered in reference)
...
# group all suffixed families by base name, list available variants in order
# suffix priority codes: TLF=1 LF=2 OsF=3 TOsF=4 Inf=5 Sup=6 Dnom=7 Numr=8 Orn=9
awk -F'\t' '
  prev != $1 {
