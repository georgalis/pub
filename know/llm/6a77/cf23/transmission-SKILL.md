...
linked, flat, bare, spread, sparse, plain, ground, steady, and single;
every claim stands iuxta its moment, the composition runs sequent with
the chronology, and the ask arrives by invite rather than by command.
...
- **oblige** --- how far the reader is obliged to read at all. High:
...
The difficulty a reader takes before the prose starts to distort is
**headroom**. It is bounded by the weaker of
...
## Invocation
    ...
    /transmission posture: fluency=0.8 stamina=0.3 appetite=0.7
      scrutiny=0.2 oblige=0.9 triage=scan
      office=construct cadence=misura priority=content
...
4. Apply scrutiny to `bare`, `flat`, `attribute`, and `invite`.
5. Apply triage to `place`, `priority`, and `single`.
6. Apply obligation to `steady`, `settled`, and `invite`.
...
|---|---|---|
| fluency | scalar | `plain`, `ground` |
| stamina | scalar | `spread`, `sparse`, `settled`, `linked`, `ground`, `sequent` |
| appetite | scalar | `spread`, `plain` --- and appetite outranks fluency |
| scrutiny | scalar | `bare`, `flat`, `iuxta`; `attribute` and `invite` below |
| triage | categorical | sets `place`, `priority`, `single` |
...
adversarial reader re-times the material whether or not invited. `ground` answers to both
...
`settled` falls with it, while `invite` rises. Nothing else moves, because voluntariness changes the reader's
...
full-scale deltas: `steady` less 0.60, `settled` less 0.35, `invite`
plus 0.30. At a shortfall of 0.60 or more, `place` moves to `early`,
...
reasonable at each point in time. Default `construct`, safe in one
...
The declaration sets floors under `iuxta` and `sequent`: 0.20 and 0.20 for
`construct`, 0.85 and 0.70 for `judicial`.

**Scrutiny is not monotone.** A settled reader wants `bare` high and
`attribute` at `artifact`. A guarded reader, evaluating the writer's
claim to authority, wants `invite` conspicuously above what standing
would justify and `bare` only moderately lowered. An adversarial reader
expects assertion and is served by low `bare`, low `flat`, `attribute`
at `agent`, and `invite` set by actual standing. It rises in
...
**Triage governs placement absolutely.** `full` takes `defer` or
...
- `sparse` and `ground` are one operation. Slowing the arrival of
...
- `sequent` and `place` trade. At 0.70 and above, composition
  ...
  carry the message: `place` loses `defer`, and `single` rises
...
- `plain` or `ground` too low ---> fluency assessed too high, usually
...
- `bare` too low outside an adversarial context ---> scrutiny assessed
  ...
  forestalls.
- `place` wrong ---> triage assessed wrong, and triage is the one
...
Standing. Low `invite` presupposes an authority held independently.
