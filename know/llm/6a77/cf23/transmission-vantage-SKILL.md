...
that was not the reason for arriving, so a voluntary reader takes a lower
value than capacity alone would indicate.
...
carried: `attribute` at `agent` names the source inside the sentence
...
`construct`: the facts are inventory for what to build next, and
...
### attribute --- where claims anchor *(mode)*
...
`attribute` answers who claims a thing and `iuxta`, above, answers as
...
- The reader tracks personalities instead of findings ---> `attribute`
...
arranged to carry the message, so `place` loses `defer` and
...
instruments, at present a cross-cutting standard within `character-selector`
