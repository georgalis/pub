...
prose is steady and single, and the ask arrives by invite rather than by
command.
...
`/transmission-land place=defer invite=0.35 steady=0.9`
...
that reader exists is `oblige`: a reader present by choice departs
...
`invite` and `place` multiply rather than add. A mandate placed early
...
### place --- where the message sits *(mode)*
...
- `defer` --- arrives after the development that makes it
...
Known full readers take `defer` or `coda`. Unknown or triage-prone
...
`defer`: the message must restate a development the document did not
order for that purpose.

## Composition Order

Decided before drafting, not after. Placement from the triage
assessment, `invite` from actual standing, the recurring position
...
- "I read the whole thing---what do you want me to do?" --->
  `place` mismatched to triage behavior, most often `defer` where
  `frame` was needed.
- "That felt like an order" ---> `invite` too low, or a low-invite
...
- "Sure, if you think so" ---> `invite` too high for what the
...
arrangement from this instrument's inventory: `place` resolves to
`coda` or `frame` and never `defer`, and `single` rises 0.15 to carry
