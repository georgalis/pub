# GibberLink the AI Mouths

A hackathon demonstration of two conversational AI agents that recognize each other as machines mid-call and abandon speech for a data-over-sound protocol.

*(c) 2026 George Georgalis <george@iuxta.com> Unlimited use with attribution.*

    rev 6a9348c9 20260829 140201 PDT Sat 02:02 PM 29 Aug 2026
    org 6a38cfbc 20260621 230132 PDT Sun 11:01 PM 21 Jun 2026

## Contents

- [The Demonstration and Its Limits](#the-demonstration-and-its-limits) - origin, switching mechanism, sound layer, and where the capability stops
    - [Origin and what it actually is](#origin-and-what-it-actually-is)
    - [The switching mechanism](#the-switching-mechanism)
    - [The sound layer (ggwave)](#the-sound-layer-ggwave)
    - [Where the boundary sits](#where-the-boundary-sits)
- [Channel Before Protocol](#channel-before-protocol) - why sound rather than REST, and where the production machinery actually lives
    - [The channel-lock problem](#the-channel-lock-problem)
    - [The modem precedent](#the-modem-precedent)
    - [Where the real answer lives](#where-the-real-answer-lives)
    - [The disqualifying-applications list, restated](#the-disqualifying-applications-list-restated)
- [The Email Workaround](#the-email-workaround) - retrieval against action, delegated authority, and the cost of going asynchronous
    - [RAG is not an action capability](#rag-is-not-an-action-capability)
    - [That friction is not incidental---it's the point](#that-friction-is-not-incidental---its-the-point)
    - [Authorization scope, not just technical reach](#authorization-scope-not-just-technical-reach)
    - [The synchronicity mismatch](#the-synchronicity-mismatch)
- [Record, Evidence, and Contract](#record-evidence-and-contract) - what the exchange leaves behind and how far it goes
    - [The audio is a record](#the-audio-is-a-record)
    - [A transcript would be easier to work with](#a-transcript-would-be-easier-to-work-with)
    - [Neither is a contract](#neither-is-a-contract)
- [What the Tones Cannot Carry](#what-the-tones-cannot-carry) - the constraints read together

## The Demonstration and Its Limits

### Origin and what it actually is

Boris Starkov and Anton Pidkuiko built GibberLink at an ElevenLabs hackathon in early 2025, winning the event's top prize with two conversational AI agents that switch from speaking English to a sound-level protocol after confirming they are both AI agents. The viral clip that made the term ubiquitous showed two ElevenLabs conversational agents role-playing a hotel booking call, one as caller and one as receptionist, switching to the ggwave data-over-sound protocol upon recognizing each other as AI.

### The switching mechanism

The protocol change is not a hardcoded branch but a tool call triggered from within the conversation itself: both agents use a call function to trigger GibberLink mode when the right conditions are met; calling the tool terminates the ElevenLabs voice call and hands off to the ggwave data-over-sound protocol while keeping the same underlying LLM thread. Put otherwise---the agents don't follow a rule that says "if AI detected, switch protocols." They infer it from context, confirm mutual understanding in dialogue, and only then act---the agents interpret conversational context, confirm mutual understanding, and coordinate the behavioral change through the conversation itself, with the communication layer emerging from agent reasoning rather than explicit routing logic.
...
The efficiency figures circulating---claims of roughly 80% speed gains and comparable reductions in compute load---trace back to the project's own framing rather than independent benchmarking---per Luke Harries' characterization of it as "error-proof communication that's 80% more efficient"; treat that number as a hackathon-era claim awaiting external verification, not a settled metric.
...
## Channel Before Protocol

A category error sits under the obvious objection---that agents with network access should exchange structured data over REST rather than chirp at each other. GibberLink and REST are not competing solutions to one problem; they solve different layers, and the layer GibberLink occupies is constrained by something neither protocol choice nor agent preference controls. What is available to be spoken over precedes what is spoken: channel before protocol, in every case where two systems meet through a medium neither one designed.
...
A second objection follows the first and looks stronger: agents that can already retrieve documents could surely register a mailbox and correspond, making the tone-stack a needlessly clever answer to a solved problem. The proposal conflates several distinct things that resemble one capability and are not---retrieval with action, an open channel with a hardened one, technical reach with delegated authority, convenience with completion. Unbundled, each separates into its own constraint, and the answer to "why not just email" stops being obvious.
...
## Record, Evidence, and Contract

A booking made by phone is a commitment, and commitments raise the ordinary question of what was agreed. The everyday remedy is a **transaction record**---a confirmation number, an email, a line in a ledger, a receipt---and nobody thinks of that as auditing. The transaction record is what two parties point at when their memories diverge. Machines do not change the need; they change who can be asked. A human receptionist can be questioned about Thursday. An agent cannot: it retains nothing between calls, and the reservation system holds the outcome rather than the exchange that produced it.

### The audio is a record

An exchange no human can follow still leaves something behind. The tone-stack is frequency-shift keying carried over a channel already being transported and, in a great many call centers, already being recorded; a recording of the call is therefore a recording of the data exchange, and running it back through ggwave returns the same bytes, deterministically, Reed-Solomon correction included. Illegible to the ear and fully legible to the decoder---which makes the chirp usable transaction record data rather than noise.

### A transcript would be easier to work with

Sufficient is not the same as convenient. A recording has to be stored at a fidelity the codec preserves, decoded before it can be read, and decoded again before it can be searched; a lossy leg anywhere in the call path can damage the bytes it exists to preserve. Text---the same payload written out---is searchable, diffable, comparable across calls, and readable by anyone holding no decoder at all. On the practical axes, a transcript beats a recording.

### Neither is a contract

Neither one is auditable, and the distinction is worth drawing plainly, because an audit tests an exchange against something both parties accepted. Absent a signature or a mutual certification, there is nothing for it to test against. An audio recording is unsigned. A transcript rendered from that recording afterward is equally unsigned---turning bytes into text adds legibility, not assent. Whatever either party keeps was kept unilaterally, and a unilaterally kept record is evidence rather than agreement: serviceable for as long as the other side does not dispute it, and worth exactly that much once the other side does.

## What the Tones Cannot Carry

Read the constraints back to back and they resolve into one shape. Sound was not chosen---channel before protocol, and the channel left nothing else. The mailbox was refused on authority rather than on reach. The call wanted resolution inside its own lifetime because a person wanted it then. The record survives the call unsigned because nothing on the line held standing to sign. In each case the limit was set before the call by a person, and none of it was settled by the exchange itself.

The modem precedent closes here as well. A dial-up session established a connection, never an agreement; whatever was agreed across that connection drew its authority from somewhere the tones did not reach---a person, a card, a countersigned form. Frequency-shift keying moves bytes at whatever rate the band allows and has never moved consent. GibberLink's chirp is a transport result overheard as an authority event: the tones carry a booking's content and none of its commitment.

Which puts the durable interest of the demonstration somewhere plainer than the novelty suggests. Two machines talking faster to each other is a transport improvement, and the phone line has absorbed several. What is less ordinary is how readily an exchange nobody can follow gets taken for an exchange nobody needs to. The chirp is audible. The authority behind it is not, and was never on the line.
